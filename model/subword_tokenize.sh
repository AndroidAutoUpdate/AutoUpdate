...
java_keywords=$(cat code_analysis/Java_keywords.txt)
...
for file in ${files[*]}
do
    subword-nmt apply-bpe -c $OUTPUT_DIR"/train.all.codes" --merges $NUM_MERGE -i $DATA_DIR"/$file" -o $OUTPUT_DIR"/$file" --num-workers -1 --glossaries $java_keywords
done
